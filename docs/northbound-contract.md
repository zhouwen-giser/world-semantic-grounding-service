...
`VALIDATE_REFERENCES` preserves fail-closed upstream validity. A reference is reusable only when the locked GOWM validation result says it is currently usable; after observing that completed result, WSGS returns `sourceOperation=VALIDATE_REFERENCES`, `revalidationRequired=false`, and a bounded cache lease in `validUntil`. The lease starts when WSGS observes the validated result, defaults to 60 seconds, and may be bounded from 1 through 300 seconds with `WSGS_REFERENCE_VALIDATION_TTL_MS`. Stale, expired, missing, or scope-denied validations receive no refreshed lease.
